...
def generateDataset(dataset, raw_file, device, snap_size, train_per, anomaly_per,sigma, x_dim, step):
    ...
    # if dist_shift != 0:
    #     print("generate artificial dist.shift:")
    #     print("edge_num before dist.shift:",len(edges))
    #     edges = degree_shift.syn_shifted_data(edges,snap_size,dist_shift)
    #     print("edge_num after dist.shift:",len(edges))
    m = len(edges)
    n = len(vertices)
    ...
    file_name_test = os.path.join(anomaly_dir, f'test_{anomaly_per}_{train_per}_{sigma}_{step}.npy')
    file_name_train = os.path.join(anomaly_dir, f'train_{anomaly_per}_{train_per}_{sigma}_{step}.npy')
    file_name_train2 = os.path.join(anomaly_dir, f'train2_{anomaly_per}_{train_per}_{sigma}_{step}.npy')
    if os.path.exists(file_name_test) == False or os.path.exists(file_name_train)==False or os.path.exists(file_name_train2)==False:
        synthetic_test, train, train2 = anomaly_generation2(train_per, anomaly_per, edges, n, m, seed=1)
        np.save(file_name_test, synthetic_test)
        np.save(file_name_train, train)
        np.save(file_name_train2, train2)
    else:
        synthetic_test = np.load(file_name_test)
        train = np.load(file_name_train)
        train2 = np.load(file_name_train2)
    train_size = int(len(train) / (snap_size) + 0.5)
    train2_size = int(len(train2) / (snap_size) + 0.5)
    test_size = int(len(synthetic_test) / (snap_size) + 0.5)
    print(train_size,train2_size, test_size)
    ...
    for ii in range(0,train_size):
        ...
        if sigma != 0:
            if x_prev_tr1 == None:
                ...
                current_x = feature_shift(x_prev_tr1, sigma)
                x_prev_tr1 = current_x
            data = Data(x=current_x, edge_index=edge_index,node_index=node_index,y=y, edge_attr=edge_attr)
        else:
            data = Data(x=x, edge_index=edge_index,node_index=node_index,y=y, edge_attr=edge_attr)
        data_list.append(data)
        train_count += 1
        print("The time step of snapshot in training set:",ii)
    for ii in range(0,train2_size):
        start_loc = ii * snap_size
        end_loc = (ii + 1) * snap_size
        edge_index = train2[start_loc:end_loc, 0:2]
        node_index = np.unique(edge_index)
        y = train2[start_loc:end_loc, 2]
        edge_attr = torch.randn(len(edge_index), 64)
        edge_index = torch.LongTensor(edge_index).t().contiguous()
        node_index = torch.LongTensor(node_index)
        y = torch.LongTensor(y)
        if sigma != 0:
            if x_prev_tr2 == None:
                ...
                current_x = feature_shift(x_prev_tr2, sigma)
                x_prev_tr2 = current_x
            data = Data(x=current_x, edge_index=edge_index,node_index=node_index,y=y, edge_attr=edge_attr)
        else:
            data = Data(x=x, edge_index=edge_index,node_index=node_index,y=y, edge_attr=edge_attr)
        data_list.append(data)
        train2_count += 1
        print("The time step of snapshot in training set 2:",ii)
    ...
    for ii in range(0,test_size):
        ...
        if sigma != 0:
            if x_prev_te == None:
                ...
                current_x = feature_shift(x_prev_te, sigma)
                x_prev_te = current_x
            data = Data(x=current_x, edge_index=edge_index,node_index=node_index,y=y, edge_attr=edge_attr)
        else:
            data = Data(x=x, edge_index=edge_index,node_index=node_index,y=y, edge_attr=edge_attr)
        data_list.append(data)
        print("The time step of snapshot in test set:",ii)

    train_size = int(len(train) / (snap_size) + 0.5)
    train2_size = int(len(train2) / (snap_size) + 0.5)
    test_size = int(len(synthetic_test) / (snap_size) + 0.5)
    print("train_count = ",train_count)
    print("train2_count = ",train2_count)
     
    return data_list, train_count, train2_count
...
def anomaly_generation2(ini_graph_percent, anomaly_percent, data, n, m,seed = 1):
    ...
    synthetic_train = generate_anomaly(data, train, n, m, 0)
    synthetic_train2 = generate_anomaly(data, train, n, m, 0.5)

    print(f'Anomaly injection takes {time.time()-t0}s.')
    return synthetic_test, synthetic_train , synthetic_train2
...
def feature_shift(x_prev, sigma):
    return x_prev + sigma * torch.randn_like(x_prev)