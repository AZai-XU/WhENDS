...
class DynamicGraphAnomaly(InMemoryDataset):
    ...
    def __init__(self, root, name, args, transform=None, pre_transform=None, pre_filter=None):
        self.dataset = name
        self.name = name + "_" + str(args.anomaly_ratio) + "_" + str(args.train_ratio) + "_" + str(args.sigma) + "_" + str(args.x_dim) + "_" + str(args.step)
        self.snap_size = args.snap_size
        self.train_ratio = args.train_ratio
        self.anomaly_ratio = args.anomaly_ratio
        self.dist_shift = args.dist_shift
        ...
        self.train2_size = torch.load(self.processed_paths[2])
    @property
    def raw_dir(self):
    ...
    def processed_file_names(self):
        return ['data.pt', 'train_size.pt', 'train2_size.pt']

    def download(self):
        if self.dataset in ['email', 'as_topology','btc_alpha', 'btc_otc']:
    ...
    def process(self):
        ...
        data_list, train_size, train2_size = generateDataset(self.dataset, raw_file, self.device, self.snap_size, self.train_ratio, self.anomaly_ratio,self.sigma, self.x_dim, self.step)
        ...
        torch.save(train2_size, self.processed_paths[2])
