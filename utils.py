...
def plot_tsne(edge_emb, y, dataset):
    os.makedirs("plot", exist_ok=True)

    edge_emb_normalized = (edge_emb)

    tsne = TSNE(n_components=2, init='random', random_state=0)
    emb_2d = tsne.fit_transform(edge_emb_normalized.detach().cpu().numpy())
    ...
    plt.scatter(
        emb_2d[normal_mask, 0],
        emb_2d[normal_mask, 1],
        c='blue',
        s=15,
        alpha=0.6,
        label='Normal'
    ...
    # 异常 — 红色
    plt.scatter(
        emb_2d[anomaly_mask, 0],
        emb_2d[anomaly_mask, 1],
        c='red',
        s=15,
        alpha=0.8,
        label='Anomaly'
    ...
    plt.title("T-SNE of Edge Embeddings")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'plot/tsne_{dataset}.pdf')
    plt.close()
...
def args_parser():
    ...
    parser.add_argument('--step', type=int, default=1, help='steps sample edges as snaphot once')
    parser.add_argument('--sigma', type=float, default=0, help='shift intensity in test/training set')

    # ===== training parameters =====
    parser.add_argument('--ae_epochs', type=int, default=250, help='training epochs')
    parser.add_argument('--ae_lr', type=float, default=0.001, help='learning rate')
    parser.add_argument('--dc_lr', type=float, default=0.00001, help='learning rate')
    parser.add_argument('--es_epochs', type=int, default=250, help='training epochs')
    parser.add_argument('--es_lr', type=float, default=0.001, help='learning rate')
    parser.add_argument('--ad_epochs', type=int, default=250, help='training epochs')
    parser.add_argument('--ad_lr', type=float, default=0.001, help='learning rate')
    parser.add_argument('--weight_decay', type=float, default=0.01, help='weight decay')

    # ===== hyper-parameters =====
    parser.add_argument('--window', type=int, default=1, help='')
    parser.add_argument('--eps', type=float, default=0.2, help='eps')
    parser.add_argument('--recon_weight', type=float, default=1, help='')
    parser.add_argument('--stats_weight', type=float, default=1, help='')
    parser.add_argument('--cons_weight', type=float, default=1, help='')
    parser.add_argument('--nce_weight', type=float, default=1, help='')
    ...
    parser.add_argument('--r_dim', type=int, default=8, help='output channels of the model')
    parser.add_argument('--expand', type=int, default=2, help='expand of inner_dim in diff_predictor')
    parser.add_argument('--EPS', type=float, default=1e-15, help='EPS')
    parser.add_argument('--noise_intense', type=float, default=0.1, help='noise_intense')
